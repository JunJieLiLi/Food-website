...
foreach($_POST as $x => $x_value) {
	//echo $x." & ".$x_value."<br>";
	if(substr($x,0,-1) == 'ingrd-t' && $x_value != ""){
		$ingredients .="<p><span class='ingrd-t'>".$x_value.", </span>";
	}
	if(substr($x,0,-1) == 'ingrd-m' && $x_value != ""){
		$ingredients .=" <span class='ingrd-m'>".$x_value."</span></p>";
	}
	if (substr($x,0,-1) == 'Step' && $x_value != ""){
		$filetext .= "<h2>".substr($x,0,-1)." ".substr($x,-1)."</h2>"."<p>".$x_value."</p>";
...
$new_img_name = $target_dir.$img_name.'.'.$extension;
...
$prep_stmt = "INSERT INTO recipe (title, user_id, recipe_content, img_directory, creat_date, ingredients) VALUES (?, ?, ?, ?, ?, ?)";
if ($insert_stmt = $mysqli->prepare($prep_stmt)){
	$insert_stmt->bind_param("sissss", $post_title, $_SESSION['user_id'], $filetext, $new_img_name, $now, $ingredients );
